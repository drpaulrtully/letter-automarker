/* =========================================================
   FEthink — Email Prompt Automarker (Level 1)
   Task 2: Sponsorship invitation prompt (RTCF)
   - Access code gate -> signed httpOnly cookie session
   - Marking rules (server-driven):
       <min words: "Please add..." only; no score; no extras; no model answer
       >=min words: score + strengths + tags + grid + improvement notes
       + Learn more tabs (collapsed by default)
       + Model AI letter dropdown (collapsed by default)
   ========================================================= */
...
// NEW: Model AI letter dropdown (separate arrow)
...
function renderModelLetter(letterText) {
  ...
  // Show wrapper, keep collapsed until learner clicks
...
function renderGrid(grid) {
  ...
  // Array-style grid support (optional)
...
async function mark() {
  ...
  try {
    ...
    // Feedback message: server may supply feedback/message; fallback safe
    ...
    // NEW: Model AI letter (second dropdown)
